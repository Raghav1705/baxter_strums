...
def skew_3d(omega):
    ...
    # YOUR CODE HERE
    w_hat = np.array([[0,-omega[2], omega[1]],
                      [omega[2], 0, -omega[0]],
                      [-omega[1], omega[0], 0]])
    return w_hat
...
def rotation_3d(omega, theta):
    ...
    sin = np.sin(mag*theta)
    cos = 1 - np.cos(mag*theta)
    square = np.linalg.matrix_power(omega_hat,2)
    R = np.add(I, omega_hat/mag*sin)
    R = np.add(R, square/(mag**2)*cos)
    return R
...
def homog_3d(xi, theta):
    ...
    if (xi[3] == 0 and xi[4] == 0 and xi[5] == 0):
        ...
        w_hat = skew_3d(w)
        w = np.reshape(w,(3,1))
        rod = (1/(np.linalg.norm(w)**2))*(np.matmul((np.eye(3)-R),np.matmul(w_hat,v))+(np.matmul(np.matmul(w,np.transpose(w)),(v*theta))))
        g = np.hstack((R,rod))
